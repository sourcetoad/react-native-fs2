...
const Example = () => {
  ...
  const copyImageToMediaStore = async (
    ...
    if (Platform.OS === 'android') {
      ...
      let contentExists;
      try {
        contentExists = await MediaStore.queryMediaStore({
          uri: '',
        ...
        console.log('this cant be catchedddddd');
      }
      ...
      if (contentExists) {
        ...
        if (overwrite) {
          // overwrite
          await MediaStore.writeToMediaFile(contentExists.uri, imagePath);
        }

        return contentExists.uri;
      }

      const contentURI = await MediaStore.copyToMediaStore(
        {
    ...
    if (Platform.OS === 'android') {
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.READ_MEDIA_IMAGES,
        {
          ...
          message:
            'RNFS2 Example App needs read/write access to your phone storage to save images',
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <Text style={styles.action}>
        - access the image and displays it below{' '}
      </Text>
      <Text style={styles.action}>
        - Note: running the example again will overwrite existing media file
      </Text>
