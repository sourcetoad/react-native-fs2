...
| `unlink` | `unlink(filepath: string): Promise<void>` | `unlink(filepath: string): Promise<void>` | 💥 **Behaviour diverges on iOS, and master's two platforms never agreed either.** Master iOS rejected `ENOENT` for a missing path (`master:ios/RNFSManager.m:213-214`). Master **Android** threw a bare `Exception("File does not exist")` (`master:RNFSManager.java:389`) which the reject helper turned into `promise.reject(null, …)` (`master:RNFSManager.java:618`) — code `null`, no `ENOENT` anywhere, so 3.x Android code matching on `ENOENT` never worked. In 4.x: iOS **returns successfully** (`ios/Fs2.swift:183-189`), Android now throws a genuine `ENOENT` (`RNFSManager.kt:357`). So iOS lost its error and Android gained the right one. 4.x Android also resolves `content://` paths before deleting (`RNFSManager.kt:356`) where master used the raw string (`master:RNFSManager.java:387`), so `unlink` on a content URI now deletes the underlying file instead of failing. The code comment at the iOS site claiming "Original library didn't throw an error here" is wrong on both platforms. Flagged as an open maintainer decision and deliberately left as-is. |
...
| Function | master (3.x) | nitro (4.x) | What changed |
|---|---|---|---|
| `getFSInfo` | `getFSInfo(): Promise<FSInfoResult>` | `getFSInfo(): Promise<FSInfoResult>` | 💥 **on Android.** iOS is unchanged: `{ totalSpace, freeSpace }` on both. Master's **Android** resolved four fields — `totalSpace`, `freeSpace`, `totalSpaceEx`, `freeSpaceEx` (`master:RNFSManager.java:549-554`) — even though `FSInfoResult` only ever declared two. Since `NativeModules` is `any`, `(await RNFS.getFSInfo() as any).freeSpaceEx` worked on 3.x Android. 4.x computes both external values and then discards them (`Fs2.kt:336-341`), so they are gone with no error. 4.x Android is more robust in one way: it guards on `MEDIA_MOUNTED` and catches (`RNFSManager.kt:398-408`), where master called `StatFs` on the external path unconditionally and could throw. |
